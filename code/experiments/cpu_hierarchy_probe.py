...
이 스크립트는 작업집합 W에 따른 메모리 계층 지연의 변화를
*실제 실리콘*에서 측정한다. 특정 R_C 값의 불연속성을 가정하지 않는다.
가속기(A100/TPU/…)가 아니라, 이 실행 환경에 실재하는 CPU 캐시 계층
(L2 → L3(LLC) → DRAM)을 대상으로 한다.

정직성 원칙(코드 cuda_backend.py와 동일):
  - 모든 값은 C 커널 내 CLOCK_MONOTONIC_RAW로 측정한 실제 시간이다.
  - 빠른 메모리 용량 C_fast 는 Linux sysfs의 마지막 레벨 캐시로 설정한다.
  - R_C = C_fast / W. 중간 점은 여러 캐시·TLB·가상화 영향으로 비단조적일 수 있다.

측정 방법: 포인터 체이싱(pointer chasing).
  크기 W 의 배열에 무작위 순열로 만든 단일 사이클 순회 링크를 심어,
  각 접근이 다음 접근 주소에 데이터 의존하도록 만들어 프리페칭과 명령어 수준
  병렬성을 제한한다. 측정값에는 캐시 외에도 TLB, 페이지 워크, 스케줄링이
  영향을 줄 수 있다.
"""
