...
wall-clock path with 12 layers, $d=512$, and five windows per point. On the GPU
(Section~\ref{sec:gpu}) the same program ran through the harness's CUDA-event
path on an NVIDIA \GpuDevice{} with 24 layers, $d=2048$, batch size 8, ten
windows, and pinned host staging; computation and host-to-device transfer were
timed separately with \texttt{torch.cuda.Event}. The harness additionally
supports XLA synchronisation for TPUs, which was not exercised here.
