...
%% \NMIframingfalse → 기존 IEEE 서사로 복귀.
%% 정식 NMI 투고본은 Nature 템플릿 빌드(./run.sh nature)를 사용하세요.
...
\ifNMIframing
  \input{./section/006_abstract_nmi_ieee.tex}  % NMI framing (IEEE syntax)
\else
  \input{./section/006_abstract.tex}           % original IEEE abstract
\fi

\input{./section/008_materials.tex}
\ifNMIframing
  \input{section/010_introduction_nmi}          % NMI-reframed introduction
\else
  \input{section/010_introduction}              % original introduction
\fi
\input{section/020_regime_principle}
\input{section/030_transfer_model}
\input{section/040_experimental_validation}
\input{section/050_implications}
\input{section/060_discussion}
\input{section/070_methods}
\input{section/080_conclusion}
