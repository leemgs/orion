...
$T_{\mathrm{total}}$ on each platform, \textsc{Orion}'s regime-aware
orchestration drives $(T_{\mathrm{comp}} + T_{\mathrm{mem}} + T_{\mathrm{swap}})$
to within 8--14\% of the physical lower bound across all five
platforms---demonstrating that the bound is near-achievable and that the
residual gap reflects excluded synchronisation physics rather than algorithmic
suboptimality (Supplementary Table~\ref{tab:achievability}).
...
mid-inference; \textsc{Orion} detects the transition and recovers $11.2 \pm 1.4\%$
of latency.
In a RAG pipeline, vector-store spikes shift $R_C$ from 0.46 to 0.58;
adaptive coordination reduces latency by $17.4 \pm 1.8\%$.
...
and the strategy ranking inversion---FlexGen +7.8\% in YOLOv8's I/O-limited
regime versus $-18.4\%$ in coordination-dominated LLM inference---confirms that
...
throughout (Supplementary Table~\ref{tab:2d_sweep}).
