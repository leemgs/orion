...
transitions; regime-aware orchestration reduces average latency by
$11.2 \pm 1.4\%$.
In a RAG pipeline (LangChain + FAISS), vector-store lookups introduce
intermittent working-set spikes that shift $R_C$ across $\theta_C$;
regime-aware adaptation reduces latency by $17.4 \pm 1.8\%$.
An edge YOLOv8 deployment transitions to the I/O-limited regime once frame
buffers exceed VRAM; reducing swap granularity by 10\% yields a
$9.5 \pm 1.2\%$ latency reduction and cuts frame-drop rate from 12\% to 2.5\%.
In all three cases the observed regime (as classified by the decision-tree
...
CO$_2$ equivalents rise from 15.8\,g to 42.5\,g per $10^6$\,tokens (IEA 2024
carbon intensity; see Table~\ref{tab:energy} caption).
